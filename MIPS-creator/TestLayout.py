...
class Test(QtWidgets.QWidget): 
	...
	def SaveSettings(self,setJS:settings): 
		testjs=settings.Test(parent=setJS)
		testjs.head_init(**self.TopRow.GetKwargs())

		self.setJSONKwargs(
			setFunction=testjs.AddUserInput,
			layout=self.allUserInput.content_area.layout(),
			allowedtype=UserInputRow
		)
		
		self.setJSONKwargs(
			setFunction=testjs.AddMemInput,
			layout=self.DataInput.content_area.layout(),
			allowedtype=DataRow
		)
		
		self.setJSONKwargs(
			setFunction=testjs.AddRegInput,
			layout=self.InputRegisters.content_area.layout(),
			allowedtype=RegisterRow
		)
		
		self.setJSONKwargs(
			setFunction=testjs.AddOutput,
